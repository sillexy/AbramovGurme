...
import { benefits, contact, directions } from "@/content/site-content";
...
const businessCards = [
  { title: "Переосмысленная традиция", text: "Адаптация принципов домашней консервации для промышленного производства.", metric: null, cta: "Честная технология" },
  { title: "Без потерь", text: "Заявление бренда: стандартные списания 5-20% становятся нулевыми.", metric: "5–20% → 0% списаний", cta: "Ноль потерь" },
  { title: "Гибкий ассортимент", text: "Заявленные брендом преимущества в планировании, логистике, широте SKU и гибкости ценообразования.", metric: "300+ SKU", cta: "Широкая линейка" },
] as const;
    ...
    <div id="top">
      ...
      <main id="main-content">
        <section className="hero hero--reference" aria-labelledby="hero-title">
          ...
          <div className="shell hero__grid">
            ...
            <div className="hero__visual">
              <SourceImage src="/products/hero-jar-v2.webp" alt="Банка с готовым блюдом в стекле" className="hero__image" priority />
              <HeroSafetyCard />
            </div>
          </div>
        </section>
        ...
        <section id="ecosystem" className="section ecosystem" aria-labelledby="ecosystem-title">
          <Reveal className="shell"><p className="eyebrow">Матрица бренда</p><h2 id="ecosystem-title">Пять векторов развития коллекции</h2><div className="flagship-band"><span>Абрамов Гурме</span><p>Флагманское направление: сложные ресторанные рецептуры и редкие деликатесные сочетания.</p></div><div className="ecosystem-grid">{directions.slice(1).map((direction) => <article key={direction.id}><span>{direction.label}</span><p>{direction.summary}</p></article>)}</div></Reveal>
        </section>
...
function HeroStats() {
  return <div className="hero__stats" aria-label="Ключевые преимущества"><div className="hero-stat"><HeroStatIcon type="jar" /><strong>5</strong><span>направлений<small>для любых задач</small></span></div><div className="hero-stat"><HeroStatIcon type="book" /><strong>400+</strong><span>рецептов<small>на любой вкус</small></span></div><div className="hero-stat"><HeroStatIcon type="leaf" /><strong>0%</strong><span>консервантов<small>и искусственных добавок</small></span></div></div>;
}

function HeroStatIcon({ type }: { type: "jar" | "book" | "leaf" }) {
  if (type === "jar") return <svg viewBox="0 0 48 48" aria-hidden="true"><path d="M17 6h14v6H17zM15 12h18l2 5v23H13V17l2-5Z" /><path d="M17 20h14M18 26c5 0 8 2 12 6M30 25c-2 5-5 8-10 9" /></svg>;
  if (type === "book") return <svg viewBox="0 0 48 48" aria-hidden="true"><path d="M5 9c7 0 13 2 19 7v26c-6-5-12-7-19-7V9Zm38 0c-7 0-13 2-19 7v26c6-5 12-7 19-7V9Z" /><path d="M24 16v26" /></svg>;
  return <svg viewBox="0 0 48 48" aria-hidden="true"><path d="M40 8C25 9 13 17 9 32c7 3 15 2 21-3 7-6 9-14 10-21Z" /><path d="M10 39c5-9 12-16 23-23" /></svg>;
}

function HeroSafetyCard() {
  return <div className="hero__safety-card"><div className="hero__safety-icon"><svg viewBox="0 0 48 48" aria-hidden="true"><path d="M24 5c6 5 12 7 18 8v11c0 10-6 16-18 20C12 40 6 34 6 24V13c6-1 12-3 18-8Z" /><path d="m16 25 5 5 11-12" /></svg></div><div className="hero__safety-copy"><strong>Безопасно. Натурально. В стекле.</strong><div><span>Срок хранения<br className="hero__shelf-life-break" />до 1 года</span><span>Сделано в России</span></div></div></div>;
}

function BenefitIcon({ index }: { index: number }) {
  const paths = [<><path d="M12 3v11.5a4.5 4.5 0 1 0 4 0V3a4 4 0 0 0-8 0v11.5a4.5 4.5 0 1 0 4 0" /><path d="M12 8v10" /><path d="M19 6h2M19 10h2M19 14h2" /></>, <><path d="M12 3s6 7 6 12a6 6 0 0 1-12 0c0-5 6-12 6-12Z" /><path d="M9.5 17c.5 1.4 1.4 2.2 2.8 2.5" /></>, <><circle cx="7" cy="17" r="2.5" /><circle cx="17" cy="7" r="2.5" /><circle cx="19" cy="18" r="2.5" /><path d="m9 15.5 6-7M9.5 17h7M18 9l1 6.5" /></>, <><path d="M9 4h6M10 4v3l-2 2v11h8V9l-2-2V4" /><path d="M8 10h8M11 14h2" /></>];
...
function BusinessIcon({ index }: { index: number }) {
  const art = [<><path d="M17 21h30l-2 25H19l-2-25Z" /><path d="M15 21h34M18 15h28v6H18zM25 29v10h14" /><path d="M49 43c5-1 8-5 9-10-5 .5-8 3-9 10ZM49 43c-2-5-5-8-9-10 0 5 3 9 9 10ZM49 43v9" /></>, <><path d="M16 48V18l14 14 9-9 11 11" /><path d="M16 48h36M23 48V31h8v17M35 48V38h8v10M47 48V34h5v14" /></>, <><path d="M14 42h36v9H14zM18 24h14v14H18zM32 30h14v14H32zM24 17h14v13H24z" /><path d="M14 47h36M25 24v5M39 30v5M31 17v5" /></>];
...
function BusinessTagIcon({ index }: { index: number }) {
  const art = [<><path d="M18 4C11 4 5 8 5 16c6 0 11-4 13-12ZM5 16c3-2 6-5 9-9" /></>, <><path d="M5 18V6h10v12H5ZM15 11h4v7h-4M8 15h4M19 5v3M19 12v3" /></>, <><path d="m12 4 7 4v8l-7 4-7-4V8l7-4ZM5 8l7 4 7-4M12 12v8" /></>];
...
function MenuCycle() {
  const dayparts = [["Завтрак", "7:00 – 10:00", "Каши длительного томления на альтернативном молоке, суфле и легкие творожные запеканки.", "Легкий старт дня"], ["Обед", "12:00 – 15:00", "Первые блюда, густые бульоны и сытные мясные блюда со сложными крупами.", "Суп + горячее"], ["Ужин", "18:00 – 21:00", "Диетическая птица, белая рыба с легкими авторскими соусами и овощи.", "Легкий вечерний рацион"], ["Ночной бар", "21:00 – 01:00", "Легкие салаты, закуски и десерты.", "Для позднего спроса"]] as const;
...
function DaypartTag({ index, label }: { index: number; label: string }) {
  const art = [<path d="M18 4C11 4 5 8 5 16c6 0 11-4 13-12ZM5 16c3-2 6-5 9-9" />, <><path d="M5 11h14l-1 7H6l-1-7Z" /><path d="M8 11V8h8v3M9 18v2M15 18v2" /></>, <><path d="M18 4C11 4 5 8 5 16c6 0 11-4 13-12ZM5 16c3-2 6-5 9-10" /><path d="M17 17h3" /></>, <><path d="M14 4a7 7 0 1 0 5.8 10.9A7 7 0 0 1 14 4Z" /><path d="m19 4 .5 1.2L21 5.7l-1.5.5L19 7.5l-.5-1.3-1.5-.5 1.5-.5L19 4Z" /></>];
